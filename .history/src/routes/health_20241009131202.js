...
router.get('/google-fit-data', passport.authenticate('session'), async (req, res) => {
  try {
    ...
    const startTimeMillis = new Date('2024-01-01').getTime();
    const endTimeMillis = Date.now(); // Current timestamp
    const response = await axios.get(`https://www.googleapis.com/fitness/v1/users/me/dataSources/${dataSourceId}/datasets/${startTimeMillis}-${endTimeMillis}`, {
      headers: {
